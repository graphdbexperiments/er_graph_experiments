...
def main():
    ...
    for index in range(len(percentage_to_update)):
        ...
        for i in range(0, runs):
            ...
            # Determine PARTSUPP nodes for key verification

            get_partsupp_amount = "PROFILE MATCH(ps:PARTSUPP) RETURN COUNT(ps) AS amount"
            partsupp_amount_query_result = new_db.execute_query_with_output_result(get_partsupp_amount)
            amount = partsupp_amount_query_result[0]['amount']

            
            activation = f"MATCH (ps:PARTSUPP) WITH ps AS partsupps, rand() AS r ORDER BY r LIMIT {round(amount * percentage_to_update[index])} SET partsupps:PARTSUPP:ACTIVATED"
            ...
            if is_relational_model or is_semirelational_model:
                index_activation = "CREATE CONSTRAINT activated_partsupp FOR (a:ACTIVATED) REQUIRE (a.ps_partkey, a.ps_suppkey) IS NODE KEY"
            ...
            if is_relational_model:
                verification = f"PROFILE MATCH (ps1:ACTIVATED), (ps2:ACTIVATED) WHERE id(ps1) < id(ps2) AND ps1.ps_partkey = ps2.ps_partkey AND ps1.ps_suppkey = ps2.ps_suppkey RETURN ps1, ps2"
            ...
            if is_graph_model:
                verification = f"PROFILE MATCH pattern1 = (s:SUPPLIER)<-[]-(ps1:ACTIVATED)-[]->(p:PART), pattern2 = (s:SUPPLIER)<-[]-(ps2:ACTIVATED)-[]->(p:PART) WHERE id(ps1) < id(ps2) RETURN pattern1, pattern2"
                current_db_hits_time = show_query_details(new_db, verification)
                current_db_hits += current_db_hits_time[0]
                current_time += current_db_hits_time[1]
            ...
            deactivation = "MATCH (ps:PARTSUPP:ACTIVATED) REMOVE ps:ACTIVATED"
            new_db.execute_query(deactivation)

            if is_relational_model or is_semirelational_model:
                index_deactivation = "DROP CONSTRAINT activated_partsupp"
