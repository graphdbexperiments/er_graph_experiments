...
def main():
    ...
    for index in range(len(percentage_to_update)):
        ...
        for i in range(0, runs):
            ...
            # get random supplier nodes for update propagation
            ...
            if percentage_to_update[index] != 'single':
                ...
                activation = f"MATCH (s:SUPPLIER) WITH s AS supps SKIP {offset} LIMIT {round(amount * percentage_to_update[index])} SET supps:SUPPLIER:ACTIVATED"
                new_db.execute_query(activation)
            else:
                offset = random.randint(0, amount-1)
                activation = f"MATCH (s:SUPPLIER) WITH s AS supps SKIP {offset} LIMIT 1 SET supps:SUPPLIER:ACTIVATED"
                new_db.execute_query(activation)
            ...
            index_activation = "CREATE CONSTRAINT activated_supp IF NOT EXISTS FOR (a:ACTIVATED) REQUIRE (a.s_suppkey) IS NODE KEY"
