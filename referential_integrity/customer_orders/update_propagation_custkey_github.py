...
def main():
    ...
    # get amount of customer nodes
    get_customer_amount = "PROFILE MATCH(c:CUSTOMER) RETURN COUNT(c) AS amount"
    customer_amount_query_result = new_db.execute_query_with_output_result(get_customer_amount)
    amount = customer_amount_query_result[0]['amount']
    ...
    for index in range(len(percentage_to_update)):
        ...
        for i in range(0, runs):
            ...
            ########## Update propagation CUSTOMER -> ORDERS


            # get customer nodes for update propagation


            if percentage_to_update[index] != 'single':
                offset = random.randint(0, amount - round(amount * percentage_to_update[index]))
                activation = f"MATCH (c:CUSTOMER) WITH c AS custs SKIP {offset} LIMIT {round(amount * percentage_to_update[index])} SET custs:CUSTOMER:ACTIVATED"
                new_db.execute_query(activation)
            else:
                offset = random.randint(0, amount-1)
                activation = f"MATCH (c:CUSTOMER) WITH c AS cust SKIP {offset} LIMIT 1 SET cust:CUSTOMER:ACTIVATED"
                new_db.execute_query(activation)
            ...
            # Enforce key constraint for CUSTOMER nodes on ACTIVATED nodes in case query planner starts with a node filter on ACTIVATED (this way index on CUSTOMER is still being used)

            index_activation = "CREATE CONSTRAINT activated_customer IF NOT EXISTS FOR (a:ACTIVATED) REQUIRE (a.c_custkey) IS NODE KEY"
            ...
            if is_relational_model:
                update = f"PROFILE MATCH (a:ACTIVATED), (o:ORDERS) WHERE o.o_custkey = a.c_custkey SET a.c_custkey = right(('00000000' + toString(a.c_custkey)), 8), o.o_custkey = right(('00000000' + toString(o.o_custkey)), 8)"
            ...
            if is_semirelational_model:
                update = f"PROFILE MATCH (a:ACTIVATED) SET a.c_custkey = right(('00000000' + toString(a.c_custkey)), 8)"
                current_db_hits_time = show_query_details(new_db, update)
                current_db_hits += current_db_hits_time[0]
                current_time += current_db_hits_time[1]
            ...
            update = "MATCH (a:ACTIVATED) SET a.c_custkey = toInteger(a.c_custkey)"
            new_db.execute_query(update)
            if is_relational_model:
                update = "MATCH (o:ORDERS) SET o.o_custkey = toInteger(o.o_custkey)"
                new_db.execute_query(update)
            ...
            index_deactivation = "DROP CONSTRAINT activated_customer"
